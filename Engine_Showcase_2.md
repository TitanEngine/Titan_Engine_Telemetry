# Titan Engine — Engine Showcase 2

[![Titan Engine ALU Stress Test](https://img.youtube.com/vi/bk8QScQG2Jk/maxresdefault.jpg)](https://youtu.be/bk8QScQG2Jk)

This showcase focuses on the **Titan Engine's** ability to handle massive computational workloads, specifically stress-testing the rendering pipeline with 1,000,000 dense procedural geometry elements and validating the core rigid-body physics solver.

This module operates entirely on a custom-built Vulkan 1.3 backend, utilizing `VK_EXT_mesh_shader` to bypass traditional vertex processing, and leverages a robust Job System to parallelize Extended Position-Based Dynamics (XPBD) calculations.

---

## Rendering Pipeline: Bypassing the Vertex Bottleneck

To render 1,000,000 active collision bodies (cubes) efficiently, the standard Vertex and Index Buffer abstractions were completely discarded. 

### Bindless BDA Transforms
Every rigid body outputs its Transform (Position, Rotation quaternion, Scale) directly into a monolithic, host-visible VRAM block. We retrieve a 64-bit Buffer Device Address (BDA) pointing to this memory and pass it to the GPU via Push Constants.

### Task & Mesh Shader Amplification
Instead of CPU-side draw calls or instancing, we dispatch Vulkan Mesh Shaders (`cmd_draw_mesh_tasks`). The GPU natively fetches the physics transforms via the BDA pointer. Each Mesh Shader workgroup procedurally generates the vertices and triangles for 20 cubes simultaneously, executing entirely on-die.

---

## Performance Telemetry & Hardware Profiling

This section contains deep-dive telemetry data, hardware profiling, and mathematical analysis of the Titan Engine. It demonstrates the direct results of low-level optimizations including zero-copy ReBAR pipelines, Temporal Gauss-Seidel solver stabilization, and bare-metal Subgroup operations.
...
* A horizontal, heavy green dashed line acts as the anchor, indicating the mathematically derived "Average FPS: ~706."
